...
export async function initializeChuteState(patientId: string): Promise<ChuteState> {
  const existingState = await db.get(KEYS.CHUTE_STATE(patientId));
  ...
  await db.set(KEYS.CHUTE_STATE(patientId), initialState);
  return initialState;
}
...
export async function getChuteState(patientId: string): Promise<ChuteState> {
  const state = await db.get(KEYS.CHUTE_STATE(patientId));
...
async function tripleLog(patientId: string, log: Omit<ChuteLog, 'logId' | 'timestamp' | 'logged'>): Promise<void> {
  ...
  const existingLogs = await db.get(KEYS.CHUTE_LOGS(patientId)) || [];
  await db.set(KEYS.CHUTE_LOGS(patientId), [...existingLogs, chuteLog]);

  // Log #2: Audit trail (separate immutable store)
  await db.set(`audit:chute:${chuteLog.logId}`, {
    ...chuteLog,
    immutable: true,
...
export async function handleBlisterPushed(
  ...
  // Save dose record
  await db.set(KEYS.DOSE_IN_PROGRESS(doseId), dose);

  // Update chute state
  state.currentDose = dose;
  await db.set(KEYS.CHUTE_STATE(patientId), state);
...
export async function handlePillsDropped(doseId: string): Promise<{ success: boolean; message: string }> {
  const dose = await db.get(KEYS.DOSE_IN_PROGRESS(doseId));
  if (!dose) {
  ...
  state.isOpen = true;
  state.currentDose = dose;
  await db.set(KEYS.CHUTE_STATE(dose.patientId), state);
...
export async function handleChuteTimeout(doseId: string): Promise<{ success: boolean; message: string; requiresCaregiverAction: boolean }> {
  ...
  const pendingActions = await db.get(KEYS.PENDING_CAREGIVER_ACTIONS(dose.patientId)) || [];
  pendingActions.push({
    doseId,
    timestamp: now,
    type: 'timeout',
  ...
  await db.set(KEYS.PENDING_CAREGIVER_ACTIONS(dose.patientId), pendingActions);
...
export async function caregiverRepresentDose(
  ...
  await db.set(KEYS.PENDING_CAREGIVER_ACTIONS(dose.patientId), updatedActions);
...
export async function getPendingCaregiverActions(patientId: string): Promise<any[]> {
  return await db.get(KEYS.PENDING_CAREGIVER_ACTIONS(patientId)) || [];
}
...
export async function getChuteLogs(patientId: string, limit: number = 100): Promise<ChuteLog[]> {
  const logs = await db.get(KEYS.CHUTE_LOGS(patientId)) || [];
