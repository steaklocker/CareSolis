...
app.post('/create-patient', async (c) => {
  try {
    ...
    await db.set(`patient:${patientId}:profile`, patientProfile);
    ...
    if (careCircle && Array.isArray(careCircle) && careCircle.length > 0) {
      ...
      await db.set(`contacts:${patientId}`, contacts);
    }
    ...
    await db.set(`legal:acknowledgments:${patientId}`, acknowledgmentRecord);
    ...
    await db.set(`rpm:enrollment:${patientId}`, rpmEnrollment);
    ...
    if (deviceId) {
      ...
      await db.set(`device:${deviceId}:config`, deviceConfig);
      await db.set(`device:${deviceId}:patient`, patientId);
    }
    ...
    await db.set(`config:${patientId}`, defaultConfig);
    ...
    const existingLogs = await db.get(`audit:${patientId}`) || [];
    await db.set(`audit:${patientId}`, [...existingLogs, auditLog]);
...
app.get('/patients', async (c) => {
  try {
    ...
    const allPatients = await db.getByPrefix('patient:');
