...
import initApp from "./init_admin.tsx";
...
async function writeAuditLog(action: string, actor: string, details: string, patientId?: string, clientTimestamp?: string, timezoneInfo?: { timezone?: string; patientLocalTime?: string }) {
    ...
    try {
        await db.set(`${prefix}${entry.timestamp}_${id}`, entry);
    } catch (error) {
...
async function writeNotificationLog(to: string, subject: string, body: string, patientId: string) {
    ...
    try {
        await db.set(`${KEYS.NOTIFICATIONS_PREFIX(patientId)}${now.getTime()}_${id}`, note);
    } catch (error) {
...
async function getSystemConfig(patientId: string = '1'): Promise<SystemConfig> {
    ...
    try {
        const stored = await db.get(KEYS.CONFIG(patientId));
        ...
        if (stored) {
            ...
            if (needsMigration) {
                ...
                await db.set(KEYS.CONFIG(patientId), safeConfig);
            }
            ...
            if (Array.isArray(safeConfig.schedule)) {
                ...
                db.set(KEYS.CONFIG(patientId), safeConfig).catch(err => {
        ...
        const medicationSchedule = await db.get("mds:medications:schedule:v1");
...
async function updateTodayEvents(events: InteractionEvent[], now?: Date, explicitDate?: string, patientId: string = '1') {
    ...
    try {
        await db.set(`${KEYS.EVENTS_PREFIX(patientId)}${today}`, events);
    } catch (error) {
...
async function getPatientName(patientId: string): Promise<string> {
    try {
        ...
        const patientData = await db.get(patientKey);
...
async function processEscalationRules(events: InteractionEvent[], config: SystemConfig, now: Date, currentTimeMinutes: number, patientId: string = '1'): Promise<boolean> {
    ...
    let rawContacts = [];
    try {
        rawContacts = await db.getByPrefix(KEYS.CONTACT_PREFIX(patientId));
    } catch (error) {
...
async function updateDeviceStateSummary(events: InteractionEvent[], systemStatus: 'nominal' | 'system_fault' = 'nominal', preloadedConfig?: SystemConfig, localTimeStr?: string, localDateStr?: string, patientId: string = '1'): Promise<DeviceState> {
    ...
    try {
        await db.set(KEYS.STATE(patientId), state);
    } catch (error) {
...
app.post("/make-server-9aeac050/api/notifications/:userId", async (c) => {
    try {
        ...
        await db.set(key, JSON.stringify(notifications));
...
app.delete("/make-server-9aeac050/api/notifications/:userId/:notificationId", async (c) => {
    try {
        ...
        await db.set(key, JSON.stringify(filtered));
        return c.json({ success: true });
    } catch (error) {
...
app.post("/make-server-9aeac050/notification-preferences", async (c) => {
    try {
        ...
        const key = `notification:preferences:${patientId}`;
        await db.set(key, JSON.stringify(preferences));
...
app.post("/make-server-9aeac050/test-notification", async (c) => {
    try {
        ...
        const contactsData = await db.get(contactsKey);
...
app.post("/make-server-9aeac050/api/seed-demo-data", async (c) => {
    try {
        ...
        await db.set(`notifications:${userId}`, JSON.stringify(demoNotifications));
        ...
        await db.set(`patient:${patientId}:profile`, JSON.stringify(demoProfile));
...
app.post("/make-server-9aeac050/api/seed-all-patients", async (c) => {
    try {
        ...
        for (const patient of patients) {
            // Seed patient profile
            await db.set(`patient:${patient.id}:profile`, JSON.stringify(patient));
            
            // Seed patient-specific system config ONLY if it doesn't exist yet
            // This prevents overwriting user-customized schedules
            const existingConfig = await db.get(KEYS.CONFIG(patient.id));
            if (!existingConfig) {
                ...
                await db.set(KEYS.CONFIG(patient.id), config);
                ...
                await db.set(KEYS.SETTINGS(patient.id), patientSettings);
            ...
            for (const contact of contacts) {
                await db.set(`${KEYS.CONTACT_PREFIX(patient.id)}${contact.id}`, contact);
            }
            ...
            await db.set(KEYS.STATE(patient.id), initialState);
    ...
    try {
        ...
        if (config.vacationMode?.enabled && config.vacationMode.endDate) {
            ...
            if (today > config.vacationMode.endDate) {
                ...
                await db.set(KEYS.CONFIG(patientId), JSON.stringify(updatedConfig));
        ...
        try {
            ...
            for (let i = 7; i <= 14; i++) {
                ...
                const existingEvents = await db.get(eventKey);
                const alreadyArchived = await db.get(archiveKey);
                
                if (existingEvents && !alreadyArchived) {
                    // Move events to archive
                    await db.set(archiveKey, existingEvents);
                    await db.del(eventKey);
...
app.get("/make-server-9aeac050/status", async (c) => {
    ...
    if (clearCache === 'true') {
        ...
        await db.del(KEYS.STATE(patientId));
    }
...
app.get("/make-server-9aeac050/events/archive", async (c) => {
    try {
        ...
        if (dateStr) {
            ...
            const archivedEvents = await db.get(archiveKey);
            ...
            const allKeys = await db.getByPrefix('events:archive:');
...
app.get("/make-server-9aeac050/logs", async (c) => {
    const patientId = c.req.query('patientId') || '1';
    const auditTrails = await db.getByPrefix(KEYS.AUDIT_PREFIX(patientId));
...
app.get("/make-server-9aeac050/contacts", async (c) => {
    const patientId = c.req.query('patientId') || '1';
    const contacts = await db.getByPrefix(KEYS.CONTACT_PREFIX(patientId));
...
app.post("/make-server-9aeac050/contacts", async (c) => {
    ...
    await db.set(`${KEYS.CONTACT_PREFIX(patientId)}${id}`, contact);
...
app.delete("/make-server-9aeac050/contacts/:id", async (c) => {
    ...
    const patientId = c.req.query('patientId') || '1';
    await db.del(`${KEYS.CONTACT_PREFIX(patientId)}${id}`);
    return c.json({ success: true });
});
...
app.get("/make-server-9aeac050/settings", async (c) => {
    ...
    const patientSettings = await db.get(KEYS.SETTINGS(patientId)) || {};
...
app.post("/make-server-9aeac050/settings", async (c) => {
    ...
    await db.set(KEYS.CONFIG(patientId), newConfig);
    ...
    if (body.patientTimezone || body.patientLocation) {
        ...
        await db.set(KEYS.SETTINGS(patientId), patientSettings);
    }
    ...
    if (newConfig.schedule && typeof newConfig.schedule === 'object' && newConfig.schedule.slots) {
        ...
        await db.set("mds:medications:schedule:v1", newConfig.schedule);
    }
...
app.post("/make-server-9aeac050/timezone", async (c) => {
    try {
        ...
        const profileRaw = await db.get(profileKey);
        ...
        await db.set(globalAuditKey, JSON.stringify(auditPayload));

        // 3. Patient-specific settings log
        const settingsLog = await db.get(KEYS.SETTINGS(patientId)) || {};
        ...
        await db.set(KEYS.SETTINGS(patientId), settingsLog);
        ...
        await db.set(profileKey, JSON.stringify(updatedProfile));
...
app.get("/make-server-9aeac050/billing-pathways", async (c) => {
    try {
        ...
        const pathways = await db.get(billingKey) || {
            ccm: false,
            bhi: false,
            apcm: false
        };
...
app.post("/make-server-9aeac050/billing-pathways", async (c) => {
    try {
        ...
        // Save to KV store
        const billingKey = 'billing:pathways:settings';
        await db.set(billingKey, pathways);

        // Verify save
        const verified = await db.get(billingKey);
...
app.post("/make-server-9aeac050/audit-log", async (c) => {
    try {
        ...
        if (patientId) {
            await db.set(`${KEYS.AUDIT_PREFIX(patientId)}${auditEntry.timestamp}_${auditEntry.id}`, auditEntry);
        }
        
        // Log 2: Global audit trail (for cross-patient analysis and FDA review)
        await db.set(`${KEYS.GLOBAL_AUDIT_PREFIX}${auditEntry.timestamp}_${auditEntry.id}`, auditEntry);
...
app.post("/make-server-9aeac050/manual-update", async (c) => {
    try {
        ...
        await db.set(`mds:manual:updates:${updateEntry.timestamp}_${updateEntry.id}`, updateEntry);
...
app.get("/make-server-9aeac050/medications", async (c) => {
    try {
        const medications = await db.get("mds:medications:v3") || [];
...
app.post("/make-server-9aeac050/medications", async (c) => {
    try {
        ...
        await db.set(KEYS.SETTINGS(patientId), settings);
...
app.get("/make-server-9aeac050/medications/schedule", async (c) => {
    try {
        const schedule = await db.get("mds:medications:schedule:v1");
...
app.post("/make-server-9aeac050/medications/schedule", async (c) => {
    try {
        ...
        // Save to KV store
        await db.set("mds:medications:schedule:v1", schedule);
        ...
        await db.set(KEYS.CONFIG(patientId), updatedConfig);
        ...
        const verifyRead = await db.get("mds:medications:schedule:v1");
...
app.get("/make-server-9aeac050/notifications", async (c) => {
    try {
        const patientId = c.req.query('patientId') || '1';
        const notes = await db.getByPrefix(KEYS.NOTIFICATIONS_PREFIX(patientId));
...
app.get("/make-server-9aeac050/dose-events", async (c) => {
    try {
        ...
        const allEvents = await db.get('mds:dose-events:v1') || [];
...
app.post("/make-server-9aeac050/dose-events", async (c) => {
    try {
        const body = await c.req.json();
        const events = await db.get('mds:dose-events:v1') || [];
        ...
        await db.set('mds:dose-events:v1', events);
...
app.post("/make-server-9aeac050/dose-events/generate-samples", async (c) => {
    try {
        ...
        await db.set('mds:dose-events:v1', sampleEvents);
...
app.get("/make-server-9aeac050/docs", async (c) => {
    const docs = await db.get(KEYS.DOCS_LIBRARY) || [];
    return c.json(docs);
});
...
app.post("/make-server-9aeac050/docs", async (c) => {
    try {
        ...
        docs.push(newDoc);
        await db.set(KEYS.DOCS_LIBRARY, docs);
...
app.delete("/make-server-9aeac050/docs/:id", async (c) => {
    ...
    await db.set(KEYS.DOCS_LIBRARY, newDocs);
    return c.json({ success: true });
});
...
app.get("/make-server-9aeac050/rpm/enrollment", async (c) => {
    try {
        const enrollment = await db.get("rpm:enrollment");
...
app.post("/make-server-9aeac050/rpm/enrollment", async (c) => {
    try {
        const body = await c.req.json();
        await db.set("rpm:enrollment", body);
        return c.json({ success: true });
    } catch (e) {
        return c.json({ error: e.message }, 500);
    }
...
app.get("/make-server-9aeac050/rpm/month/:month", async (c) => {
    try {
        ...
        const activities = await db.get(`rpm:activities:${month}`) || [];
        ...
        const allEvents = await db.getByPrefix(KEYS.EVENTS_PREFIX);
        const allLogs = await db.getByPrefix(KEYS.AUDIT_PREFIX);
        ...
        const medications = await db.get("medications:assignments") || [];
...
app.post("/make-server-9aeac050/rpm/activity", async (c) => {
    try {
        ...
        await db.set(`rpm:activities:${month}`, activities);
...
app.post("/make-server-9aeac050/rpm/consent", async (c) => {
    try {
        ...
        await db.set(`rpm:consent:${consent.patientName}`, consent);
        ...
        if (enrollment) {
            ...
            await db.set("rpm:enrollment", enrollment);
        }
...
app.get("/make-server-9aeac050/rpm/consent/:patientName", async (c) => {
    try {
        ...
        const consent = await db.get(`rpm:consent:${patientName}`);
...
app.post("/make-server-9aeac050/device-malfunction-report", async (c) => {
    try {
        ...
        await db.set(`malfunction:${report.id}`, report);
        
        // Add to device-specific malfunction log
        const deviceReports = await db.get(`malfunction:device:${report.deviceId}`) || [];
        deviceReports.push(report.id);
        await db.set(`malfunction:device:${report.deviceId}`, deviceReports);
        ...
        if (report.fdaReportingRequired) {
            ...
            const fdaQueue = await db.get('malfunction:fda-queue') || [];
            fdaQueue.push({
                reportId: report.id,
                queuedAt: new Date().toISOString(),
                status: 'pending_fda_submission'
            });
            await db.set('malfunction:fda-queue', fdaQueue);
        }
...
app.get("/make-server-9aeac050/device-malfunction-reports", async (c) => {
    try {
        ...
        const allReports = await db.getByPrefix('malfunction:');
...
app.get("/make-server-9aeac050/device-malfunction-reports/:deviceId", async (c) => {
    try {
        ...
        const reportIds = await db.get(`malfunction:device:${deviceId}`) || [];
        
        // Fetch all reports for this device
        const reports = [];
        for (const id of reportIds) {
            const report = await db.get(`malfunction:${id}`);
...
app.get("/make-server-9aeac050/caregiver-manual", async (c) => {
    try {
        const manual = await db.get("caregiver:manual");
...
app.post("/make-server-9aeac050/caregiver-manual", async (c) => {
    try {
        const body = await c.req.json();
        await db.set("caregiver:manual", body);
        return c.json({ success: true });
    } catch (e) {
        return c.json({ error: e.message }, 500);
    }
...
app.get("/make-server-9aeac050/systems-infrastructure", async (c) => {
    try {
        const doc = await db.get("systems:infrastructure");
...
app.post("/make-server-9aeac050/systems-infrastructure", async (c) => {
    try {
        const body = await c.req.json();
        await db.set("systems:infrastructure", body);
        return c.json({ success: true });
    } catch (e) {
        return c.json({ error: e.message }, 500);
    }
...
app.route('/make-server-9aeac050/init', initApp);
...
app.get("/make-server-9aeac050/medication-escalation-configs", async (c) => {
    try {
        const configs = await db.get("mds:escalation:configs:v1") || [];
...
app.post("/make-server-9aeac050/medication-escalation-configs", async (c) => {
    try {
        ...
        await db.set("mds:escalation:configs:v1", configs);
        ...
        const changeLogs = await db.get("mds:escalation:change_log:v1") || [];
        changeLogs.push(changeLog);
        await db.set("mds:escalation:change_log:v1", changeLogs.slice(-1000)); // Keep last 1000
...
app.get("/make-server-9aeac050/escalation-overrides", async (c) => {
    try {
        const overrideLogs = await db.get("mds:escalation:overrides:v1") || [];
...
app.post("/make-server-9aeac050/escalation-overrides", async (c) => {
    try {
        ...
        await db.set("mds:escalation:overrides:v1", overrideLogs);
        ...
        const complianceLogs = await db.get("mds:compliance:log:v1") || [];
        complianceLogs.push(complianceLog);
        await db.set("mds:compliance:log:v1", complianceLogs.slice(-5000)); // Keep last 5000
...
app.get("/make-server-9aeac050/ai-adherence-score", async (c) => {
    try {
        const score = await db.get("mds:ai:adherence:current:v1") || null;
...
app.get("/make-server-9aeac050/ai-adherence-history", async (c) => {
    try {
        const history = await db.get("mds:ai:adherence:history:v1") || [];
...
app.post("/make-server-9aeac050/ai-adherence-override", async (c) => {
    try {
        ...
        // Get existing override logs
        const overrideLogs = await db.get("mds:ai:overrides:v1") || [];
        ...
        await db.set("mds:ai:overrides:v1", overrideLogs);
        ...
        const complianceLogs = await db.get("mds:compliance:log:v1") || [];
        complianceLogs.push(complianceLog);
        await db.set("mds:compliance:log:v1", complianceLogs.slice(-5000));
...
app.get("/make-server-9aeac050/system-integration-overview", async (c) => {
    try {
        ...
        const metrics = await db.get("mds:system:metrics:v1") || null;
...
app.get("/make-server-9aeac050/integration-logs", async (c) => {
    try {
        const logs = await db.get("mds:integrations:approval_logs:v1") || [];
...
app.post("/make-server-9aeac050/integration-approve", async (c) => {
    try {
        ...
        await db.set("mds:integrations:approval_logs:v1", updatedLogs);
        ...
        const complianceLogs = await db.get("mds:compliance:integration_approvals:v1") || [];
        complianceLogs.push({
            timestamp: new Date().toISOString(),
        ...
        await db.set("mds:compliance:integration_approvals:v1", complianceLogs);
...
app.post("/make-server-9aeac050/integration-revoke", async (c) => {
    try {
        ...
        // Log 3: Compliance record
        const complianceLogs = await db.get("mds:compliance:integration_revocations:v1") || [];
        complianceLogs.push({
            timestamp: new Date().toISOString(),
        ...
        await db.set("mds:compliance:integration_revocations:v1", complianceLogs);
...
// ============================================================================
// REFILLS MONITOR — calendar-based, no device telemetry
// KV keys:
//   med_fill:patient:{patientId}          → MedFill record
//   refill_request:{uuid}                 → RefillRequest record
//   refill_requests:patient:{patientId}   → latest RefillRequest id
// ============================================================================

const REFILL_LEAD_DAYS = 7;
const PATIENT_IDS = ['1', '2', '3'];

interface MedFill {
  id: string;
  patient_id: string;
  fill_date: string;          // ISO date YYYY-MM-DD
  pouches_loaded: number;
  pouches_per_day: number;
  recommended_fill_days: number;
  rx_refills_remaining: number;
  rx_expiry: string;          // ISO date
  pharmacy_name: string;
  pharmacy_contact: string;
  status: string;             // OK | Refill due | Urgent | Requested | Confirmed | Filled
  updated_at: string;
}
...
interface RefillRequest {
  ...
  patient_id: string;
  med_fill_id: string;
  pouches_to_order: number;
  pharmacy_name: string;
  status: string;             // Requested | Confirmed | Filled
  requested_at: string;
  confirmed_at: string | null;
  filled_at: string | null;
}

function computeRefillStatus(fill: MedFill): {
  daysSupplied: number;
  daysElapsed: number;
  ...
  const daysSupplied = Math.floor(fill.pouches_loaded / fill.pouches_per_day);
  const fillDate = new Date(fill.fill_date + 'T00:00:00Z');
  const today = new Date();
  today.setUTCHours(0, 0, 0, 0);
  const daysElapsed = Math.floor((today.getTime() - fillDate.getTime()) / 86400000);
  const daysRemaining = daysSupplied - daysElapsed;

  const runOutDate = new Date(fillDate);
  runOutDate.setUTCDate(runOutDate.getUTCDate() + daysSupplied);
  const runOutDateStr = runOutDate.toISOString().split('T')[0];

  let status = fill.status;
  // Only auto-update status if not already in a terminal/in-progress state
  if (!['Requested', 'Confirmed', 'Filled'].includes(fill.status)) {
    if (daysRemaining <= 3) status = 'Urgent';
    else if (daysRemaining <= REFILL_LEAD_DAYS) status = 'Refill due';
    else status = 'OK';
  }

  // Renewal check: no refills left OR expiry within 30 days
  const expiryDate = new Date(fill.rx_expiry + 'T00:00:00Z');
  const daysToExpiry = Math.floor((expiryDate.getTime() - today.getTime()) / 86400000);
  const renewalNeeded = fill.rx_refills_remaining === 0 || daysToExpiry <= 30;
  const renewalReason = fill.rx_refills_remaining === 0
    ? 'No refills remaining'
    : `Rx expires in ${daysToExpiry} days`;

  const recommendedFillDays = fill.recommended_fill_days || 28;
  const pouchesToOrder = fill.pouches_per_day * Math.min(recommendedFillDays, daysSupplied);

  return { daysSupplied, daysElapsed, daysRemaining, runOutDate: runOutDateStr, status, renewalNeeded, renewalReason, pouchesToOrder };
}

// Seed default med_fill rows for the 3 known patients if none exist.
// Provides realistic demo states: OK / Refill due / Urgent.
async function seedMedFillIfAbsent(patientId: string) {
  const existing = await db.get(`med_fill:patient:${patientId}`);
  if (existing) return;
  const today = new Date();
  const seedMap: Record<string, Partial<MedFill>> = {
    '1': { fill_date: fmtDate(addDays(today, -18)), pouches_loaded: 84, pouches_per_day: 3, recommended_fill_days: 28, rx_refills_remaining: 2, rx_expiry: fmtDate(addDays(today, 180)), pharmacy_name: 'Santa Rosa Pharmacy', pharmacy_contact: '(707) 555-1001' },
    '2': { fill_date: fmtDate(addDays(today, -24)), pouches_loaded: 75, pouches_per_day: 3, recommended_fill_days: 28, rx_refills_remaining: 1, rx_expiry: fmtDate(addDays(today, 25)), pharmacy_name: 'Portland ME Compounding', pharmacy_contact: '(207) 555-2002' },
    '3': { fill_date: fmtDate(addDays(today, -5)),  pouches_loaded: 84, pouches_per_day: 3, recommended_fill_days: 28, rx_refills_remaining: 0, rx_expiry: fmtDate(addDays(today, 60)), pharmacy_name: 'Austin MedRx', pharmacy_contact: '(512) 555-3003' },
  };
  const seed = seedMap[patientId];
  if (!seed) return;
  const fill: MedFill = {
    id: `mf-seed-${patientId}`,
    patient_id: patientId,
    fill_date: seed.fill_date!,
    pouches_loaded: seed.pouches_loaded!,
    pouches_per_day: seed.pouches_per_day!,
    recommended_fill_days: seed.recommended_fill_days!,
    rx_refills_remaining: seed.rx_refills_remaining!,
    rx_expiry: seed.rx_expiry!,
    pharmacy_name: seed.pharmacy_name!,
    pharmacy_contact: seed.pharmacy_contact!,
    status: 'OK',
    updated_at: new Date().toISOString(),
  };
  await db.set(`med_fill:patient:${patientId}`, fill);
}
...
// GET /refills — all patients with computed status, sorted by daysRemaining asc
app.get('/make-server-9aeac050/refills', async (c) => {
  try {
    // Seed missing fills so demo always has data
    await Promise.all(PATIENT_IDS.map(id => seedMedFillIfAbsent(id)));

    const fills = (await db.mget(PATIENT_IDS.map(id => `med_fill:patient:${id}`))).filter(Boolean) as MedFill[];
    const patientNames: Record<string, string> = { '1': 'Eleanor Whitmore', '2': 'Margaret Chen', '3': 'Robert Martinez' };

    const rows = fills.map(fill => {
      const computed = computeRefillStatus(fill);
      return { ...fill, ...computed, patientName: patientNames[fill.patient_id] ?? `Patient ${fill.patient_id}` };
    });

    rows.sort((a, b) => a.daysRemaining - b.daysRemaining);
    return c.json({ patients: rows });
  } catch (e) {
    console.error('[refills] GET error:', e);
    return c.json({ error: String(e) }, 500);
  }
});

// POST /refills/med-fill — upsert a med_fill record for a patient
app.post('/make-server-9aeac050/refills/med-fill', async (c) => {
  try {
    const body = await c.req.json();
    const { patient_id, fill_date, pouches_loaded, pouches_per_day, recommended_fill_days, rx_refills_remaining, rx_expiry, pharmacy_name, pharmacy_contact } = body;
    if (!patient_id || !fill_date || !pouches_loaded || !pouches_per_day) {
      return c.json({ error: 'patient_id, fill_date, pouches_loaded, pouches_per_day are required' }, 400);
    }
    const existing: MedFill | null = await db.get(`med_fill:patient:${patient_id}`);
    const fill: MedFill = {
      id: existing?.id ?? `mf-${patient_id}-${Date.now()}`,
      patient_id,
      fill_date,
      pouches_loaded,
      pouches_per_day,
      recommended_fill_days: recommended_fill_days ?? 28,
      rx_refills_remaining: rx_refills_remaining ?? 0,
      rx_expiry: rx_expiry ?? fmtDate(addDays(new Date(), 365)),
      pharmacy_name: pharmacy_name ?? '',
      pharmacy_contact: pharmacy_contact ?? '',
      status: 'OK',
      updated_at: new Date().toISOString(),
    };
    await db.set(`med_fill:patient:${patient_id}`, fill);
    return c.json({ success: true, fill });
  } catch (e) {
    console.error('[refills] med-fill POST error:', e);
    return c.json({ error: String(e) }, 500);
  }
});

// POST /refills/request — create a refill request and stub pharmacy transport
app.post('/make-server-9aeac050/refills/request', async (c) => {
  try {
    const body = await c.req.json();
    const { patient_id } = body;
    if (!patient_id) return c.json({ error: 'patient_id required' }, 400);

    const fill: MedFill | null = await db.get(`med_fill:patient:${patient_id}`);
    if (!fill) return c.json({ error: 'No med_fill record found for this patient' }, 404);

    const { pouchesToOrder } = computeRefillStatus(fill);
    const requestId = `rr-${patient_id}-${Date.now()}`;
    const request: RefillRequest = {
      id: requestId,
      patient_id,
      med_fill_id: fill.id,
      pouches_to_order: pouchesToOrder,
      pharmacy_name: fill.pharmacy_name,
      status: 'Requested',
    ...
    // Decrement rx_refills_remaining
    const updatedFill: MedFill = {
      ...fill,
      rx_refills_remaining: Math.max(0, fill.rx_refills_remaining - 1),
      status: 'Requested',
      updated_at: new Date().toISOString(),
    };
    ...
    await db.mset([
      ...
      { key: `refill_requests:patient:${patient_id}`, value: requestId },
      { key: `med_fill:patient:${patient_id}`, value: updatedFill },
    ]);

    // TODO: replace stub with real pharmacy channel integration
    // e.g. POST to pharmacy API, HL7 FHIR MedicationRequest, or fax service
    console.log(`[refills] STUB pharmacy POST for patient ${patient_id}: ${pouchesToOrder} pouches → ${fill.pharmacy_name} (${fill.pharmacy_contact})`);

    return c.json({ success: true, request, updatedFill });
  } catch (e) {
    console.error('[refills] request POST error:', e);
    return c.json({ error: String(e) }, 500);
  }
});

// POST /refills/filled — pharmacy confirmed fill; resets the countdown
app.post('/make-server-9aeac050/refills/filled', async (c) => {
  try {
    const body = await c.req.json();
    const { patient_id, pouches_loaded, fill_date } = body;
    if (!patient_id || !pouches_loaded) return c.json({ error: 'patient_id, pouches_loaded required' }, 400);

    const fill: MedFill | null = await db.get(`med_fill:patient:${patient_id}`);
    if (!fill) return c.json({ error: 'No med_fill record found' }, 404);

    const newFill: MedFill = {
      ...fill,
      fill_date: fill_date ?? fmtDate(new Date()),
      pouches_loaded,
      status: 'OK',
      updated_at: new Date().toISOString(),
    };

    // Update latest request to Filled if one exists
    const latestReqId: string | null = await db.get(`refill_requests:patient:${patient_id}`);
    if (latestReqId) {
      const req: RefillRequest | null = await db.get(`refill_request:${latestReqId}`);
      if (req) {
        await db.set(`refill_request:${latestReqId}`, { ...req, status: 'Filled', filled_at: new Date().toISOString() });
      }
    }

    await db.set(`med_fill:patient:${patient_id}`, newFill);
    const computed = computeRefillStatus(newFill);
    return c.json({ success: true, fill: { ...newFill, ...computed } });
  } catch (e) {
    console.error('[refills] filled POST error:', e);
    return c.json({ error: String(e) }, 500);
  }
});
