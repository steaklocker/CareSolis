...
async function detectAnomalies(userId: string, deviceInfo: any): Promise<any[]> {
  ...
  const accessLogs = await db.getByPrefix(`auth:access_log:user:${userId}`);
  ...
  const knownDevices = await db.getByPrefix(`auth:device:user:${userId}`);
...
// SIGNUP - TEMPORARILY OPEN FOR INITIAL SETUP
// TODO: Re-enable admin-only access after initial user creation
authApp.post('/signup', async (c) => {
  try {
    // ADMIN CHECK TEMPORARILY DISABLED FOR INITIAL SETUP
    // Uncomment below to re-enable admin-only signup
    /*
    const authHeader = c.req.header('Authorization');
    const token = authHeader?.split(' ')[1];

    // Check if it's an admin session (not the public anon key)
    const sessions = await db.getByPrefix('auth:session:');
    const adminSession = sessions.find((s: any) => s.id === token && s.isActive);

    if (!adminSession) {
      return c.json({ error: 'Unauthorized - Active admin session required' }, 401);
    }

    // Verify admin role
    const adminUser = await db.get(`auth:user:${adminSession.userId}`);
    if (!adminUser || adminUser.role !== 'system_admin') {
      return c.json({ error: 'Forbidden - System admin role required' }, 403);
    }
    */

    console.log('📝 Signup request received - admin check temporarily disabled for initial setup');

    const { email, password, name, role = 'primary_caregiver' } = await c.req.json();
    ...
    const securitySettings = await db.get('auth:security_settings');
    const minLength = securitySettings?.passwordMinLength || 12;

    if (password.length < minLength) {
      return c.json({ error: `Password must be at least ${minLength} characters` }, 400);
    }
    ...
    const user = {
      ...
      mfaEnabled: false,
      mfaMethod: null,
      passwordExpiresAt: securitySettings?.passwordExpirationDays
        ? new Date(Date.now() + securitySettings.passwordExpirationDays * 86400000).toISOString()
        : null
    };

    // Save user
    await db.set(`auth:user:${userId}`, user);
    await db.set(`auth:user:email:${email}`, user);

    // Log user creation
    await db.set(`auth:audit_log:user_created:${userId}`, {
      userId,
      action: 'user_created',
...
authApp.post('/login', async (c) => {
  try {
    ...
    // Get user by email
    const users = await db.getByPrefix('auth:user:email:');
    ...
    if (!isValidPassword) {
      ...
      await db.set(`auth:user:${userRecord.id}`, userRecord);
    ...
    if (anomalies.length > 0) {
      ...
      for (const anomaly of anomalies) {
        ...
        await db.set(`auth:anomaly:${alertId}`, {
          id: alertId,
    ...
    if (mfaRequired) {
      ...
      await db.set(`auth:mfa:${userRecord.id}`, {
        userId: userRecord.id,
...
authApp.post('/verify-mfa', async (c) => {
  try {
    ...
    const mfaRecord = await db.get(`auth:mfa:${userId}`);
    ...
    if (!isValidCode) {
      ...
      await db.set(`auth:mfa:${userId}`, mfaRecord);
      return c.json({ error: 'Invalid MFA code' }, 401);
    }
    
    // MFA verified - get user and create session
    const userRecord = await db.get(`auth:user:${userId}`);
    
    // Get device info from temporary storage (set during login)
    const deviceInfo = await db.get(`auth:temp_device:${userId}`);
    
    const session = await createSession(userRecord, deviceInfo);
    
    // Clean up
    await db.del(`auth:mfa:${userId}`);
    await db.del(`auth:temp_device:${userId}`);
...
authApp.post('/logout', async (c) => {
  try {
    ...
    const session = await db.get(`auth:session:${sessionId}`);
    
    if (session) {
      session.isActive = false;
      await db.set(`auth:session:${sessionId}`, session);
...
authApp.get('/session', async (c) => {
  try {
    ...
    const user = await db.get(`auth:user:${session.userId}`);
...
authApp.get('/access-logs', async (c) => {
  try {
    ...
    let logs = await db.getByPrefix('auth:access_log:');
...
authApp.get('/anomaly-alerts', async (c) => {
  try {
    ...
    let alerts = await db.getByPrefix('auth:anomaly:');
...
authApp.post('/resolve-anomaly', async (c) => {
  try {
    ...
    const alert = await db.get(`auth:anomaly:${anomalyId}`);
    ...
    await db.set(`auth:anomaly:${anomalyId}`, alert);
...
authApp.get('/security-settings', async (c) => {
  try {
    let settings = await db.get('auth:security_settings');
    
    if (!settings) {
      // Return default settings
      settings = getDefaultSecuritySettings();
      await db.set('auth:security_settings', settings);
    }
...
authApp.post('/audit-log', async (c) => {
  try {
    ...
    await db.set(`auth:audit_log:${logId}`, {
      id: logId,
      ...entry,
...
authApp.get('/audit-trail', async (c) => {
  try {
    ...
    let logs = await db.getByPrefix('auth:audit_log:');
...
async function createSession(user: any, deviceInfo: any) {
  ...
  await db.set(`auth:session:${sessionId}`, session);
  await db.set(`auth:session:user:${user.id}:${sessionId}`, session);
...
async function registerDevice(userId: string, deviceInfo: any) {
  const existingDevice = await db.get(`auth:device:${deviceInfo.deviceId}`);
  
  if (existingDevice) {
    existingDevice.lastSeen = new Date().toISOString();
    existingDevice.accessCount += 1;
    await db.set(`auth:device:${deviceInfo.deviceId}`, existingDevice);
  } else {
    const device = {
      deviceId: deviceInfo.deviceId,
      userId,
    ...
    await db.set(`auth:device:${deviceInfo.deviceId}`, device);
    await db.set(`auth:device:user:${userId}:${deviceInfo.deviceId}`, device);
  }
}
...
async function logAccessEvent(
  ...
  await db.set(`auth:access_log:${logId}`, log);
  
  if (userId) {
    await db.set(`auth:access_log:user:${userId}:${logId}`, log);
  }
}
