...
export function ProtectedRoute({ children, allowedRoles, redirectTo = '/' }: ProtectedRouteProps) {
  ...
  if (!role || !user) {
    console.error('🔒 ProtectedRoute: role or user is undefined!');
    return (
      <div className="flex items-center justify-center min-h-[50vh]">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-emerald-500 mx-auto mb-4"></div>
  ...
  // Check role-based route access using the centralized function
  const hasRouteAccess = canAccessRoute(user.role as any, location.pathname);
  const isAllowed = allowedRoles.includes(role) && hasRouteAccess;
